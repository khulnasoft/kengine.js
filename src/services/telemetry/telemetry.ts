...
function initializeConf() {
  try {
    return new Conf({ projectName: "kengine" });
  } catch (_) {
    return null;
  }
}
...
function notify() {
  ...
  console.log(`${chalk.cyan.bold("Attention")}: Kengine now collects completely anonymous telemetry regarding usage. This is used to guide our roadmap.`);
  console.log("You can learn more, including how to opt-out of this anonymous program, by heading over to:");
  console.log("https://kengine.khulnasoft.com/docs/cli/anonymous-telemetry");
  console.log();
  record("downloaded", {});
}
