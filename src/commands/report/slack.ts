...
export const desc = "Post a Kengine report to Slack";
...
export const builder: CommandBuilder<Options, Options> = (yargs) => {
  return yargs
    .options({
      ...
      path: {
        type: "string",
        desc: "Path to the Kengine output file",
        required: false,
      },
    })
    .example([
      ...
      # Post a report to Slack with explicit path to Kengine output file:
      $0 slack --channel <channel> --path <path-to-kengine-output>
      `,
      ],
    ])
...
export async function handler(argv: Arguments<Options>) {
  ...
  if (!workspace.slackTeamId) {
    s.fail("Please install the Kengine slack app and follow the setup instructions to send reports to Slack.");
    console.log("Docs: https://kengine.khulnasoft.com/docs/integrations/slack/overview/");
    return;
  }
