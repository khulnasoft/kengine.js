...
export async function promptForEmail(): Promise<string> {
  ...
  const { accountEmail } = await prompt<{ accountEmail: string }>({
    type: "input",
    name: "accountEmail",
    message: "What email address did you sign up to Kengine with?",
    required: true,
...
export async function promptRedirectToCloudFormation(): Promise<boolean> {
  const { confirm } = await prompt<{ confirm: boolean }>({
    ...
    name: "confirm",
    initial: true,
    message: "Press [Enter] to be redirected to CloudFormation and deploy the Kengine connector stack",
  });
